...
bool testa(idNum *lista_nova, int len){
  ...
  for(int i = 0; i < len; i++){
    if (lista_nova[i].Val - (&lista_original)[i].Val != 1){
      resp = false;
      printf("a lista nova tem %d enquanto a original teria %d na posição %d \n", lista_nova[i].Val, (&lista_original)[i].Val, i);
      return resp;
    }
...
int main(int argc, char* argv[]){
  ...
  printf("Fim da threado principal \n");
